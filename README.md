...
[![Version](https://img.shields.io/badge/versi%C3%B3n-v0.9.0-blue.svg)](https://github.com/khitisakamachisa-collab/cmmsbioai)
...
**Versión actual:** v0.9.0 — Prototipo Funcional en Desarrollo Activo
...
| v0.9.0 | **RF01 Universal + Modo TEST + Usuarios** | Rediseño mayor del modelo `Equipo` (RF01): eliminados `registro_sanitario_bolivia`, `calibracion_proxima`, `responsable_tecnico_id`, `proveedor_principal` (texto). Agregados `observaciones`, `fecha_inicio_garantia`, `condicion_origen` (enum de 9 valores), `proveedor_principal_id` (FK a `Proveedor`). Cambios de obligatoriedad: `nombre_corto` → NOT NULL, `fecha_adquisicion` → opcional. Campos `modelo`, `marca`, `numero_serie` NO editables después de creado (protegen carpetas y `.meta.json`). DELETE con verificación de dependencias (OTs, documentos, MP, historial). Import-excel actualizado con creación automática de proveedores al vuelo. Endpoint `/equipos/from-proveedor-nombre` para crear proveedor desde el formulario de equipo. **3 usuarios TEST**: `admin/admin`, `tech/tech`, `user/user`. **Modo TEST**: botones "Cargar datos TEST" (40 registros coherentes) y "Limpiar BD" en Configuración. Badge "🧪 MODO TEST" en navbar. `proxima_fecha` en TareaPreventiva ahora es editable (no auto-calculada). Widget "MPs Próximos (7 días)" en dashboard. Plantilla Excel/CSV regenerada con nuevos campos. |
