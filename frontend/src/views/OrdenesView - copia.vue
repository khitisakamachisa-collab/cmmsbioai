...
<template>
  <div class="dashboard-container">
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="showModal = false">
      <div class="modal" style="width: 750px; max-width: 95vw;">
        ...
        <form @submit.prevent="saveOrden">
          ...
          <!-- Fila 2: Título, Fecha/Hora, Tiempo -->
          <div class="form-row-3">
            ...
            <div class="form-group"><label>Fecha y Hora de Creación</label><input v-model="formData.fecha_creacion" type="datetime-local"></div>
    ...
    <div v-if="showViewModal" class="modal-overlay" @click.self="showViewModal = false">
      <div class="modal modal-view-ot" style="width: 750px; max-width: 95vw;">
        <h3 style="margin: 0 0 1rem 0; color: #1e293b; border-bottom: 2px solid #e2e8f0; padding-bottom: 0.5rem;">Detalle de Orden #{{ selectedOT.id }}</h3>
        ...
        <div style="max-width: 520px; margin: 0 auto;">
          <div class="detail-table-block" style="flex: none; margin-bottom: 1rem;">
            ...
            <table class="detail-table">
              <tbody>
                <tr><td class="detail-label">Equipo</td><td>{{ getEquipoNombre(selectedOT.equipo_id) }}</td></tr>
                <tr><td class="detail-label">Título / Tipo</td><td>{{ selectedOT.titulo }}</td></tr>
                <tr><td class="detail-label">Prioridad</td><td><span class="badge" :class="prioridadClass(selectedOT.prioridad)">{{ selectedOT.prioridad }}</span></td></tr>
                <tr><td class="detail-label">Estado</td><td><span class="badge" :style="{ backgroundColor: getEstadoColor(selectedOT.estado_id) }">{{ estadosOT.find(e => e.id === selectedOT.estado_id)?.nombre_estado }}</span></td></tr>
                <tr v-if="selectedOT.orden_preventiva_id"><td class="detail-label">Origen</td><td><span class="badge badge-preventivo">Preventivo #{{ selectedOT.orden_preventiva_id }}</span></td></tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="detail-table-block" style="flex: none;">
            ...
            <table class="detail-table">
              <tbody>
                <tr><td class="detail-label">Técnico</td><td>{{ getTecnicoNombre(selectedOT.tecnico_asignado_id) }}</td></tr>
                <tr><td class="detail-label">Fecha Creación</td><td>{{ selectedOT.fecha_creacion ? formatFechaHora(selectedOT) : 'N/A' }}</td></tr>
                <tr><td class="detail-label">Tiempo Invertido</td><td>{{ selectedOT.tiempo_real_invertido || 0 }} {{ selectedOT.unidad_tiempo === 'dias' ? 'días' : 'horas' }}</td></tr>
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div class="detail-descriptions">
          ...
          <div v-if="selectedOT.acciones_realizadas" class="desc-col">
            <h4>Acciones Realizadas</h4>
            <div class="description-box">{{ selectedOT.acciones_realizadas }}</div>
          </div>
        </div>

        <div class="detail-section-title">Costos Adicionales ({{ detalleCostos.length }})</div>
        <div v-if="!detalleCostos.length" class="empty-section">Sin costos registrados.</div>
        <table v-else class="detail-table detail-table-full">
          <thead>
            <tr>
              <th class="detail-th">Tipo</th>
              <th class="detail-th">Descripción</th>
              <th class="detail-th">Monto</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="c in detalleCostos" :key="c.id">
              <td><span class="badge" style="background: #7c3aed; font-size: 0.72rem; padding: 1px 8px; border-radius: 10px;">{{ c.tipo_costo }}</span></td>
              <td>{{ c.descripcion_costo }}</td>
              <td style="font-weight: 700; color: #5b21b6; white-space: nowrap;">Bs. {{ Number(c.monto_costo).toFixed(2) }}</td>
            </tr>
            <tr style="background: #ede9fe; border-top: 2px solid #7c3aed;">
              <td colspan="2" style="font-weight: 700; text-align: right; padding: 0.4rem 0.6rem;">Total:</td>
              <td style="font-weight: 700; color: #5b21b6; font-size: 1rem; padding: 0.4rem 0.6rem;">Bs. {{ totalDetalleCostos.toFixed(2) }}</td>
            </tr>
          </tbody>
        </table>

        <div class="detail-section-title">Repuestos Utilizados</div>
        <div v-if="!selectedOT.repuestos_usados || !selectedOT.repuestos_usados.length" class="empty-section">Sin repuestos registrados.</div>
        <table v-else class="detail-table detail-table-full">
          <thead>
            <tr>
              <th class="detail-th">Cantidad</th>
              <th class="detail-th">Repuesto</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="rep in selectedOT.repuestos_usados" :key="rep.repuesto_id">
              <td>{{ rep.cantidad_utilizada }} x</td>
              <td>{{ getRepuestoNombre(rep.repuesto_id) }}</td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="modal modal-edit-ot" style="width: 800px; max-width: 95vw;">
        ...
        <form @submit.prevent="updateOrden">
          ...
          <div class="form-row-4">
            ...
            <div class="form-group"><label>Técnico Asignado</label><select v-model="editFormData.tecnico_asignado_id"><option :value="null">-- Sin Asignar --</option><option v-for="tec in tecnicos" :key="tec.id" :value="tec.id">{{ tec.full_name || tec.username }}</option></select></div>
          </div>
          <!-- Fila 2: Título, Fecha/Hora, Tiempo -->
          <div class="form-row-3">
            <div class="form-group">
            ...
            <div class="form-group"><label>Fecha y Hora de Creación</label><input v-model="editFormData.fecha_creacion" type="datetime-local"></div>
            <div class="form-group">
              <label>Tiempo Invertido</label>
              <div style="display: flex; gap: 8px;">
...
.detail-section-title { font-size: 0.95rem; font-weight: 600; color: #2c3e50; margin-bottom: 0.5rem; margin-top: 0.8rem; }
.detail-descriptions { display: flex; gap: 1.5rem; margin-bottom: 0.5rem; }
.desc-col { flex: 1; }
...
/* Costos + Repuestos en la misma fila (modal Editar) */
.costos-repuestos-row { display: flex; gap: 1.25rem; }
.cr-col { flex: 1; min-width: 0; }
@media (max-width: 768px) { .costos-repuestos-row { flex-direction: column; } }
</style>